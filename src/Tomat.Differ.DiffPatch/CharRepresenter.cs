    ...
    public class CharRepresenter {
        ...
        public virtual string WordsToChars(string line) {
            ...
            for (int i = 0, len; i < line.Length; i += len) {
                ...
                len = 1;
                if (char.IsLetter(c))
                    while (i + len < line.Length && char.IsLetterOrDigit(line, i + len))
                        len++;
                else if (char.IsDigit(c))
                    while (i + len < line.Length && char.IsDigit(line, i + len))
                        len++;
                else if (c == ' ' || c == '\t')
                    while (i + len < line.Length && line[i + len] == c)
                        len++;
        ...
        public string LinesToChars(IEnumerable<string> lines) => new string(
            lines.Select(AddLine)
                .ToArray()
        );
