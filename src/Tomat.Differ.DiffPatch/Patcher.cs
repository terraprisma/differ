    ...
    public class Patcher {
        ...
        public class Result {
            ...
            public string Summary() {
                ...
                if (mode == Mode.FUZZY) {
                    ...
                    return $"FUZZY: {patch.Header} quality {q}%" + (offset > 0 ? $" offset {offset} lines" : "");
                }
        ...
        private class WorkingPatch : Patch {
            ...
            public void WordsToChars(CharRepresenter rep) {
                wmContext = ContextLines.Select(rep.WordsToChars)
                    .ToArray();

                wmPatched = PatchedLines.Select(rep.WordsToChars)
                    .ToArray();
            }
        }
        ...
        public Patcher(IEnumerable<Patch> patches, IEnumerable<string> lines, CharRepresenter charRep = null) {
            this.patches = patches.Select(p => new WorkingPatch(p))
                .ToList();
        ...
        private void WordsToChars() {
            ...
            wmLines = lines.Select(charRep.WordsToChars)
                .ToList();
        }
        ...
        private Patch ApplyExactAt(int loc, WorkingPatch patch) {
            ...
            int patchedDelta = patches.Where(p => p.KeepoutRange2?.end <= loc)
                .Sum(p => p.AppliedDelta.Value);
        ...
        public static Patch AdjustPatchToMatchedLines(Patch patch, int[] match, IReadOnlyList<string> lines) {
            ...
            for (int i = 0, j = 0, ploc = -1; i < patch.length1; i++) {
                ...
                if (mloc >= 0 && ploc >= 0 && mloc - ploc > 1) {
                    ...
                    var op = diffs[j - 1].op == Operation.DELETE && diffs[j].op == Operation.DELETE ? Operation.DELETE : Operation.EQUAL;
        ...
        private (int[] match, float score) FindMatch(int loc, IReadOnlyList<string> wmContext) {
            ...
            var keepoutRanges = patches.Select(p => p.KeepoutRange2)
                .Where(r => r != null)
                .Select(r => r.Value);

            // parts of file to search in
            var ranges = new LineRange { length = wmLines.Count }.Except(keepoutRanges)
                .ToArray();
        ...
        public static (int[] match, float score) FuzzyMatch(IReadOnlyList<string> wmPattern, IReadOnlyList<string> wmText, int loc, FuzzyMatchOptions options = default, LineRange[] ranges = default) {
            ...
            var fwdMatchers = ranges.Select(r => new MatchMatrix(wmPattern, wmText, options.MaxMatchOffset, r))
                .SkipWhile(m => loc > m.WorkingRange.last)
                .ToArray();

            var revMatchers = ranges.Reverse()
                .Select(r => new MatchMatrix(wmPattern, wmText, options.MaxMatchOffset, r))
                .SkipWhile(m => loc < m.WorkingRange.first)
                .ToArray();
        ...
        private struct MatchRunner {
            ...
            public bool Step(ref float bestScore, ref int[] bestMatch) {
                ...
                while (active.end < mms.Length
                       && mms[active.end]
                           .WorkingRange.Contains(loc))
                    active.end++;
