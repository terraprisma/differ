...
namespace Tomat.Differ.DotnetPatcher.Utility {
    public class DirectoryUtility {
        ...
        public static IEnumerable<(string file, string relPath)> EnumerateSrcFiles(string dir) => EnumerateFiles(dir)
            .Where(
                f => !f.relPath.Split('/', '\\')
                    .Any(nonSourceDirs.Contains)
            );
        ...
        public static string GetOutputPath(string path, PEFile module) {
            ...
            var rootNamespace = AssemblyUtility.GetAssemblyTitle(module);
            ...
            path = new StringBuilder(path).Replace(".", "/", 0, stopFolderzingAt)
                .ToString();
        ...
        public static IEnumerable<(string file, string relPath)> EnumerateFiles(string dir) => Directory.EnumerateFiles(dir, "*", SearchOption.AllDirectories)
